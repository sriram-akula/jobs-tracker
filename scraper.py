...
NAV_TIMEOUT_MS = 35_000   # per-page load timeout (raised from 25s)
CONCURRENCY = 6           # parallel browser pages (be polite, keep this modest)
EXTRA_SETTLE_MS = 4_000   # fixed extra wait after load, for slow client-rendered boards
SCROLL_PASSES = 4         # number of scroll-to-bottom passes to trigger lazy/infinite-scroll lists
...
async def collect_all_frame_text(page) -> str:
    """
    Read visible text from the MAIN frame AND every embedded iframe on the
    page. Many career pages render their actual job list inside an iframe
    pointing at a separate ATS (Greenhouse/Lever/etc.) - reading only the
    main frame's body misses that entirely, which was the biggest source
    of false "no matching role found" results in the first run.
    """
    chunks = []
    for frame in page.frames:
        try:
            txt = await frame.locator("body").inner_text(timeout=3_000)
            if txt:
                chunks.append(txt)
        except Exception:
            continue  # frame not ready / cross-origin block / no body - skip, don't fail the whole check
    return "\n".join(chunks)


async def check_company(browser, name: str, url: str, sem: asyncio.Semaphore, debug: bool = False):
    async with sem:
        result = {
            "company": name,
            "url": url,
        ...
        try:
            ...
            # let client-rendered job boards start fetching their data
            try:
                await page.wait_for_load_state("networkidle", timeout=10_000)
            except Exception:
                pass

            # trigger lazy-load / infinite-scroll lists by scrolling down repeatedly
            ...
            # fixed settle time - covers slow API calls that finish after networkidle gives up
            await page.wait_for_timeout(EXTRA_SETTLE_MS)

            text = await collect_all_frame_text(page)

            if debug:
                DEBUG_DIR.mkdir(exist_ok=True)
                safe_name = re.sub(r"[^A-Za-z0-9_-]+", "_", name)[:60]
                (DEBUG_DIR / f"{safe_name}.txt").write_text(text or "(empty - nothing captured)", encoding="utf-8")
...
async def run(debug: bool = False):
    ...
    async with async_playwright() as p:
        ...
        tasks = [check_company(browser, name, url, sem, debug=debug) for name, url in COMPANIES]
...
if __name__ == "__main__":
    import os
    debug_mode = os.environ.get("DEBUG_DUMP", "false").lower() == "true"
    asyncio.run(run(debug=debug_mode))
